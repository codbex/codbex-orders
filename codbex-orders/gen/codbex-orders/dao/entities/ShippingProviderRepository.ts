...
export class ShippingProviderRepository {
    ...
    private async triggerEvent(data: ShippingProviderEntityEvent | ShippingProviderUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-orders-entities-ShippingProvider", ["trigger"]);
        ...
        producer.topic("codbex-orders-entities-ShippingProvider").send(JSON.stringify(data));
    }
}
