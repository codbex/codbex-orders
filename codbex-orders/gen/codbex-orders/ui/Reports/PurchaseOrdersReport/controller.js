angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	.config(['EntityServiceProvider', (EntityServiceProvider) => {
		EntityServiceProvider.baseUrl = '/services/ts/codbex-orders/gen/codbex-orders/api/Reports/PurchaseOrdersReportService.ts';
	}])
	.controller('PageController', ($scope,, EntityService, LocaleService) => {
		...
		//-----------------Events-------------------//
		Dialogs.addMessageListener({ topic: 'codbex-orders.Reports.PurchaseOrdersReport.filter', handler: (data) => {
			$scope.filter = data;
			$scope.loadPage(1);
		}});
		//-----------------Events-------------------//
