			...
			EntityService.count($scope.filter).then((resp) => {
				...
				EntityService.list($scope.filter).then((response) => {
					...
					myChart.data.datasets = [
						{
							label: LocaleService.t('codbex-orders:codbex-orders-model.t.PURCHASEORDERSTOTALREPORT_TOTAL', 'Total'),
					...
					Dialogs.showAlert({
						title: LocaleService.t('codbex-orders:codbex-orders-model.t.PURCHASEORDERSTOTALREPORT'),
						message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToLF', { name: '$t(codbex-orders:codbex-orders-model.t.PURCHASEORDERSTOTALREPORT)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-orders:codbex-orders-model.t.PURCHASEORDERSTOTALREPORT'),
					message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToCount', { name: '$t(codbex-orders:codbex-orders-model.t.PURCHASEORDERSTOTALREPORT)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
