angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		let translated = {
			yes: 'Yes',
			no: 'No',
			deleteConfirm: 'Are you sure you want to delete WorkOrder? This action cannot be undone.',
			deleteTitle: 'Delete WorkOrder?'
		};
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-orders:codbex-orders-model.defaults.deleteTitle', { name: '$t(codbex-orders:codbex-orders-model.t.WORKORDER)' });
			translated.deleteConfirm = LocaleService.t('codbex-orders:codbex-orders-model.messages.deleteConfirm', { name: '$t(codbex-orders:codbex-orders-model.t.WORKORDER)' });
		});
		$scope.dataPage = 1;
		$scope.dataCount = 0;
		$scope.dataOffset = 0;
		...
		Extensions.getWindows(['codbex-orders-custom-action']).then((response) => {
			$scope.pageActions = response.data.filter(e => e.perspective === 'WorkOrder' && e.view === 'WorkOrder' && (e.type === 'page' || e.type === undefined));
		});
		...
		Dialogs.addMessageListener({ topic: 'codbex-orders.WorkOrder.WorkOrder.entityCreated', handler: () => {
			refreshData();
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-orders.WorkOrder.WorkOrder.entityUpdated', handler: () => {
			refreshData();
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-orders.WorkOrder.WorkOrder.entitySearch', handler: (data) => {
			resetPagination();
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToLF', { name: '$t(codbex-orders:codbex-orders-model.t.WORKORDER)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-orders:codbex-orders-model.t.WORKORDER'),
					message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToCount', { name: '$t(codbex-orders:codbex-orders-model.t.WORKORDER)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
		...
		$scope.selectEntity = (entity) => {
			...
			Dialogs.postMessage({ topic: 'codbex-orders.WorkOrder.WorkOrder.entitySelected', data: {
				entity: entity,
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-orders:codbex-orders-model.t.WORKORDER'),
							message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToDelete', { name: '$t(codbex-orders:codbex-orders-model.t.WORKORDER)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
		...
		$scope.optionsWorkerValue = (optionKey) => {
		...
		$scope.optionsSalesOrderValue = (optionKey) => {
