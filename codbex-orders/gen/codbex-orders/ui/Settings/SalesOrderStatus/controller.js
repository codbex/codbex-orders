angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-orders:codbex-orders-model.defaults.deleteTitle', { name: '$t(codbex-orders:codbex-orders-model.t.SALESORDERSTATUS)' });
			translated.deleteConfirm = LocaleService.t('codbex-orders:codbex-orders-model.messages.deleteConfirm', { name: '$t(codbex-orders:codbex-orders-model.t.SALESORDERSTATUS)' });
		});
			...
			EntityService.count(filter).then((resp) => {
				...
				request.then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToLF', { name: '$t(codbex-orders:codbex-orders-model.t.SALESORDERSTATUS)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-orders:codbex-orders-model.t.SALESORDERSTATUS'),
					message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToCount', { name: '$t(codbex-orders:codbex-orders-model.t.SALESORDERSTATUS)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then((response) => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-orders:codbex-orders-model.t.SALESORDERSTATUS'),
							message: LocaleService.t('codbex-orders:codbex-orders-model.messages.error.unableToDelete', { name: '$t(codbex-orders:codbex-orders-model.t.SALESORDERSTATUS)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
