...
export class SentMethodRepository {
    ...
    private async triggerEvent(data: SentMethodEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-orders/OrdersSettings/SentMethod", ["trigger"]);
        ...
        producer.queue("codbex-orders/OrdersSettings/SentMethod").send(JSON.stringify(data));
    }
}